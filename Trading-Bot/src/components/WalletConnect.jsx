...
const WalletConnect = ({ onClose }) => {
  ...
  const updateAvailability = () => {
    try {
      const result = checkWalletAvailability();
  ...
  const wallets = [
    ...
    },
    {
      ...
      icon: Gem,
      description: "NFT Marketplace & Ordinals Wallet",
    },
  ];
    ...
    <>
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-[9999] backdrop-blur-md bg-black/60 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full shadow-2xl p-6 transition-all duration-300">
            {/* Header */}
            ...
            <div className="space-y-3">
              {wallets.map((wallet) => {
                ...
                // Wallet available → show connect
                return (
                  <button
                    key={wallet.id}
                    onClick={() => handleConnect(wallet.id)}
                    disabled={isConnecting}
                    className={`w-full bg-gradient-to-r ${wallet.gradient} hover:opacity-90 text-white font-bold py-4 px-6 rounded-lg transition-all duration-300 flex items-center justify-between disabled:opacity-50 disabled:cursor-not-allowed group border-0`}
                  >
                    <div className="flex items-center gap-3">
